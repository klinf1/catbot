...
class PileCommandHandler(CommandBase):
    ...
    async def send_pile_message(self):
        ...
        if not char:
            ...
            await self.bot.send_message(self.chat_id, "Этот персонаж заморожен!")
        elif char.is_dead:
            await self.bot.send_message(self.chat_id, "Этот персонаж мертв!")
        elif not char.clan_no:
            await self.bot.send_message(self.chat_id, "Этот персонаж не принадлежит ни одному клану!")
        else:
