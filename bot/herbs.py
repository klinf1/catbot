...
class HerbCommandHandler(CommandBase):
    ...
    async def gather(self):
        ...
        try:
            self.write_log("debug", f"Начало собирательства для {self.user.username} {params}")
            herb, success = HerbUser(params[0], (params[1] if len(params) > 1 else None)).gather()
        except CharacterDeadException:
            await self.context.bot.send_message(self.chat_id, "Этот персонаж мертв!")
            self.write_log("info", f"Собирательство с мертвым персонажем: {self.user.username}")
        except CharacterFrozenException:
            await self.context.bot.send_message(self.chat_id, "Этот персонаж сейчас неактивен!")
            self.write_log("info", f"Собирательство с замороженным персонажем: {self.user.username}")
        except Exception as err:
            self.write_log("error", err)
        else:
            if success and herb:
                await self.context.bot.send_message(self.chat_id, f"Вы нашли {herb.name}!")
            elif not herb:
                await self.bot.send_message(self.chat_id, "Вы не нашли никакой полезной травы.")
            else:
