...
class HerbCommandHandler(CommandBase):
    ...
    async def gather(self):
        ...
        try:
            main_logger.debug(f"Начало собирательства для {self.user.username} {params}")
            ...
            main_logger.info(f"Собирательство с мертвым персонажем: {self.user.username}")
        except CharacterFrozenException:
            await self.context.bot.send_message(self.chat_id, "Этот персонаж сейчас неактивен!")
            main_logger.info(f"Собирательство с замороженным персонажем: {self.user.username}")
