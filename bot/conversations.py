...
from bot.buttons import get_view_inv_keyboard, get_hunt_keyboard, get_single_inv_keyboard
    ...
    async def action(self):
        ...
        match self.query_data:
            case "view_inv":
                ...
                await self.bot.send_message(self.chat_id, f"Инвентарь кота {char.name}", reply_markup=get_view_inv_keyboard(char.no))
            case "clear_inv":
...
class InvViewConv(CallbackBase):
    ...
    async def action(self):
        ...
        await self.bot.send_message(self.chat_id, text, reply_markup=get_single_inv_keyboard())
