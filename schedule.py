...
def create_schedules() -> None:
    ...
    if not store.get_all_jobs():
        ...
        scheduler.add_job(advance_seasons, 'cron', name="advance_seasons", second=1)  # day=dt.now().day
        ...
        scheduler.add_job(reset_hunt_attempts, 'cron', name="reset_hunt_attempts", second=2)  # day_of_week=dt.now().weekday()
